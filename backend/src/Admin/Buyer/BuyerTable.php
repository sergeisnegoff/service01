...
class BuyerTable  extends TableComponent
{
     ...
     * @title Поставщики
     * @header
     * {{ button('Добавить организацию', {type: 'success', size: 'sm', icon: 'plus'}) | open('BuyerEditor') }}
     ...
     *      {% else %}
     *          {{ title | open('BuyerEditor', {key: _key}) | controls(buttons(button('', {size: 'xs', icon: 'arrow-up'}) | tooltip('Выгрузить список ВСД') | action('importVsd', _key))) }}
     *      {% endif %}
     * @col {{ inn }}
     * @col {{ kpp }}
     * @col {{ status }}
    ...
    protected function filter(
        ...
        $query->filterByType(Company::TYPE_BUYER);
