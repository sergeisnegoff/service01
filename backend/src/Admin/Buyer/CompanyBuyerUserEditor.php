...
class CompanyBuyerUserEditor extends EditorComponent
{
    /**
     * @title Покупатель организации
     *
     * @entity CompanyUser
     *
     * @field active
     ...
     * @template
     * {% filter row %}
     *     {{ company_id | external('BuyerTable', { empty: 'Выберите организацию', query: { external:true } }) | group('Организация') | col(12) }}
    ...
    public function preSave(ComponentRequest $request, ComponentResponse $response, $entity)
    {
        ...
        $userGroup = UserGroupQuery::create()
            ->filterByName(UserGroup::GROUP_BUYER)
            ->findOne();

        $rel = UserGroupRelQuery::create()
            ->filterByUserId($entity->getUserId())
            ->filterByUserGroup($userGroup)
