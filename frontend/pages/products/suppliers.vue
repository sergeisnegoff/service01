                    ...
                    <div class="row">
                        ...
                        >
                            <div class="wraapper__button-nowrap content__justify-end">
                                <div class="btn">
                                    <button
                                        :disabled="!suppliers.data || !canAdd"
                                        @click="add"
                                    >
                        ...
                        <div class="col-12">
                            <div class="box__content__table">
                                ...
                                <ListController
                                    ...
                                    :fetch-api="fetchProductManufacturers"
...
<script>
import  { addProductManufacturers, fetchProductManufacturers, changeProductManufacturers, deleteProductManufacturers } from '@/api/product';
...
export default {
    ...
    computed: {
        canAdd() {
            return !this.suppliers.data.items.find(item => !('id' in item));
        }
    },
    methods: {
        add() {
            this.suppliers.data.items.unshift(makeItemEditable({
                title: ''
            }));
            this.suppliers.data.items[0].isEdit = true;
        },
        async fetchProductManufacturers(...config) {
            const data = await fetchProductManufacturers(...config);

            return this.normalizeProductManufacturer(data);
        },
        normalizeProductManufacturer(data) {
            return {
                ...data,
        ...
        getItemIndex(item) {
            return this.suppliers.data.items.findIndex(_item => item.id ? _item.id === item.id : item === item);
        },
        async save(item) {
            ...
            if (item.id) {
                await changeProductManufacturers(item.id, {
                    title: this.formData.title
                });
            } else if (this.formData.title) {
                await addProductManufacturers({
                    title: this.formData.title
                });
            }
        ...
        async doDeleteItem(item) {
            ...
            if (item.id) {
                await deleteProductManufacturers(item.id);
                await this.$router.push({ query: { page: 1 } });
                this.$refs.listController.fetchData();
            } else {
                this.suppliers.data.items.splice(index, 1);
            }
        },
        doEditItem(item) {
